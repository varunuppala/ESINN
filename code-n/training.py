...
def train(logging_params=None, report_freq=100, cont_data_gen=True, dataset_params=None, train_loader=None, val_loader=None,transfer = None):
...
def train_one_epoch(model, train_loader, val_loader, optimizer, criterion, report_params, logging_params):
	...
	for i, (images, labels) in enumerate(train_loader):
		...
		if method == "regression":
			...
			train_loss += loss.item()
			train_correct+= int(torch.sum(prediction==labels.data))
			train_pts += len(outputs)
			
			epoch_train_correct+= int(torch.sum(prediction==labels.data))
			epoch_train_loss += loss.item()
			epoch_train_pts+=len(images)
			...
			with torch.no_grad():
				for images, labels in val_loader:
					...
					if method == "regression":
						...
						val_loss += loss.item()
						val_correct+= int(torch.sum(prediction==labels.data))
						val_pts += len(outputs)
